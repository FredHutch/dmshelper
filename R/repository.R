...
repository_chunk <- function(input){
  if (determine_cores(input)$genomics_flag) {
    ...
    if (input$human_subjects) {
      repository_chunk_temp <- yaml.load_file("template/genomics.yml")$repository_hs
    } else {
      repository_chunk_temp <- yaml.load_file("template/genomics.yml")$repository_non_hs
    }
  } else {
...
findable_chunk <- function(input){
  if (determine_cores(input)$genomics_flag) {
    #####
    if (input$human_subjects) {
      findable_chunk_temp <- yaml.load_file("template/genomics.yml")$findable
    } else {
      findable_chunk_temp <- yaml.load_file("template/genomics.yml")$findable
    }
  } else {
...
duration_chunk <- function(input){
  if (determine_cores(input)$genomics_flag) {
    #####
    if (input$human_subjects) {
      duration_chunk_temp <- yaml.load_file("template/genomics.yml")$duration_hs
    } else {
      duration_chunk_temp <- yaml.load_file("template/genomics.yml")$duration_non_hs
    }
  } else {
